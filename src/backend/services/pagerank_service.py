...
class PageRankService:
    ...
    def __init__(self, config: PageRankConfig, logger: ILogger):
        ...
        self.full_pagerank_df = pd.DataFrame(
    ...
    def initial_data_load(self):
        ...
        self.logger.info(
            f"DEBUG (Service): initial_data_load called. Using pagerank_csv_path from self.config: {self.config.output_analysis_path}"
        )
        self.logger.info(
            f"DEBUG (Service): initial_data_load called. Using link_graph_csv_path from self.config: {self.config.input_edge_list_path}"
        )
        # --- END DEBUG PRINTS ---
        ...
        try:
            ...
            if not all(
                col in temp_pagerank_df.columns for col in required_cols_pagerank
            ):
                raise ValueError(
                    f"PageRank data missing required columns. Found: {temp_pagerank_df.columns.tolist()}"
                )

            self.full_pagerank_df = temp_pagerank_df.copy()
            self.full_pagerank_df["Folder_Depth"] = (
                pd.to_numeric(self.full_pagerank_df["Folder_Depth"], errors="coerce")
                .fillna(-1)
                .astype(int)
            )
            self.full_pagerank_df["PageRank"] = pd.to_numeric(
                self.full_pagerank_df["PageRank"], errors="coerce"
            )
            self.full_pagerank_df.dropna(subset=["PageRank"], inplace=True)

            self.pagerank_analyzer_instance = PageRankGraphAnalyzer(
                self.full_pagerank_df.copy()
            )
            initial_load_status.append(
                f"✅ PageRank data loaded from {self.config.output_analysis_path}"
            )
        except FileNotFoundError:
            initial_load_status.append(
                f"⚠️ PageRank CSV not found at {self.config.output_analysis_path}. Please run analysis to generate it."
            )
            self.pagerank_analyzer_instance = None
            self.logger.warning(
                f"PageRank CSV not found: {self.config.output_analysis_path}"
            )
        except (IOError, ValueError) as e:
            initial_load_status.append(
                f"❌ Error loading/parsing PageRank CSV: {e}. File might be empty or malformed."
            )
            self.pagerank_analyzer_instance = None
            self.logger.error(f"PageRank CSV load/parse failed: {e}")
        except Exception as e:
            initial_load_status.append(
                f"❌ Unexpected error during PageRank CSV load: {e}"
            )
        ...
        try:
            ...
            if not all(
                col in temp_link_graph_df.columns for col in required_cols_link_graph
            ):
                raise ValueError(
                    f"Link Graph data missing required columns. Found: {temp_link_graph_df.columns.tolist()}"
                )

            self.full_link_graph_df = temp_link_graph_df.copy()

            if not self.full_link_graph_df.empty:
                self.hits_analyzer_instance = HITSGraphAnalyzer(
                    self.full_link_graph_df.copy(), self.full_pagerank_df.copy()
                )
                initial_load_status.append(
                    f"✅ Link graph data loaded from {self.config.input_edge_list_path}"
                )
            else:
                initial_load_status.append(
                    f"⚠️ Link graph CSV at {self.config.input_edge_list_path} is empty. HITS analysis will not be possible."
                )
                self.hits_analyzer_instance = None
                self.logger.warning(
                    f"Link graph CSV is empty: {self.config.input_edge_list_path}"
                )

        except FileNotFoundError:
            initial_load_status.append(
                f"⚠️ Link Graph CSV not found at {self.config.input_edge_list_path}. Please run link extraction to generate it."
            )
            self.hits_analyzer_instance = None
            self.logger.warning(
                f"Link Graph CSV not found: {self.config.input_edge_list_path}"
            )
        except (IOError, ValueError) as e:
            initial_load_status.append(
                f"❌ Error loading/parsing Link Graph CSV: {e}. File might be empty or malformed."
            )
            self.hits_analyzer_instance = None
            ...
            initial_load_status.append(
                f"❌ Unexpected error during Link Graph CSV load: {e}"
            )
    ...
    def process_and_save_pagerank(self):
        ...
        try:
            if self.full_link_graph_df.empty or self.full_link_graph_df.shape[1] < 2:
                raise ValueError(
                    "Link graph data is not loaded or is empty for PageRank processing."
                )
            ...
            os.makedirs(
                os.path.dirname(self.config.output_analysis_path), exist_ok=True
            )
    ...
    def perform_analysis(self, analysis_type: str, depth_level: int, top_n: int):
        ...
        if self.full_pagerank_df.empty and analysis_type == "PageRank":
            self.logger.warning(
                "PageRank DataFrame is empty, attempting to re-run initial data load for PageRank analysis."
            )
        ...
        if self.full_link_graph_df.empty and analysis_type == "HITS":
            self.logger.warning(
                "Link Graph DataFrame is empty, attempting to re-run initial data load for HITS analysis."
            )
            self.initial_data_load()  # Now called without arguments, using self.config
        ...
        if analysis_type == "PageRank":
            if self.pagerank_analyzer_instance is None or self.full_pagerank_df.empty:
                ...
                self.logger.error(
                    "PageRank analyzer is not initialized or data is empty."
                )
                new_headers = ["URL", "Folder_Depth", "PageRank"]
                results_df = pd.DataFrame(columns=new_headers)
            else:
                try:
                    filtered_df = self.full_pagerank_df[
                        self.full_pagerank_df["Folder_Depth"] == depth_level
                    ].copy()
                    ...
                    if results_df.empty:
                        ...
                        self.logger.info(
                            "No PageRank candidates found for filtering criteria."
                        )
                    else:
            ...
            if self.hits_analyzer_instance is None or self.full_link_graph_df.empty:
                status_msg = (
                    "HITS analysis not possible. Graph data not loaded or empty."
                )
                self.logger.error(
                    "HITS graph analyzer is not initialized or data is empty."
                )
