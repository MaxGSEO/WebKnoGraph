...
class DataLoader:
    ...
    def stream_unprocessed_data(self, processed_urls: set, batch_size: int):
        ...
        input_glob_path = os.path.join(self.input_path, "**", "*.parquet").replace(os.sep, "/")
        ...
        if processed_urls:
            ...
            self.con.register("processed_urls_df_table", pd.DataFrame(list(processed_urls), columns=["URL"]))
