...
class RecommendationEngine:
    ...
    def __init__(self, config: LinkPredictionConfig, logger: ILogger, url_processor: URLProcessor):
        self.config = config
        self.logger = logger
    ...
    def get_recommendations(self, source_url: str, top_n: int = 20, min_folder_depth: int = 0, max_folder_depth: int = 10):
        ...
        for i in range(k):
            ...
            if dest_idx >= num_nodes:
                 self.logger.warning(f"Skipping recommendation candidate with out-of-bounds index: {dest_idx} (max_valid_index={num_nodes-1})")
                 continue
            ...
            is_existing_link = (source_idx, dest_idx) in self.existing_edges or (dest_idx, source_idx) in self.existing_edges
        ...
        final_recommendations_df = pd.DataFrame(recommendations).sort_values(by="SCORE", ascending=False).head(top_n)

        if final_recommendations_df.empty:
            return None, "No recommendations found matching the criteria (filters, existing links, etc.). Try adjusting filters or source URL."
