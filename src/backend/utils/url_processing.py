    ...
    def get_folder_depth(url: str) -> int:
        ...
        try:
            ...
            if not isinstance(url, str):
                logger.error(f"Input URL is not a string type: {type(url)}. Value: {url}")
            ...
            if not path or path == '/':
                logger.debug(f"Path is empty or root for URL: '{url}', returning 0 depth.")
                return 0

            segments = [s for s in path.strip('/').split('/') if s]

            logger.debug(f"URL: '{url}', Segments: {segments}, Calculated Depth: {len(segments)}")

            return len(segments)
        except Exception:
            logger.exception(f"CRITICAL ERROR calculating folder depth for URL: '{url}'. Exception details:") # Use logger.exception to print full traceback
