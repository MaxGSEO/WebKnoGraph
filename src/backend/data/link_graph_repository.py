...
class LinkGraphStateManager: # Renamed from StateManager
        ...
        if os.path.exists(self.edge_list_path):
            try:
                ...
                if not edge_df.empty:
                    ...
                    all_urls_in_graph = pd.concat([edge_df['FROM'], edge_df['TO']]).unique()
                    visited_urls = set(all_urls_in_graph)
                self.logger.info(f"Rebuilt visited set with {len(visited_urls)} URLs from edge list CSV.")
            except Exception as e:
                self.logger.warning(f"Could not rebuild visited set from edge list CSV (may be a new crawl or empty file): {e}")
        return visited_urls
    ...
    def append_edges_to_csv(self, df_edges: pd.DataFrame):
        ...
        try:
            df_edges.to_csv(self.edge_list_path, mode='a', header=write_header, index=False)
